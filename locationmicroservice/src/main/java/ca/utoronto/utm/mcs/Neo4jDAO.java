...
public class Neo4jDAO {
    ...
    public Result getUserLocationByUid(String uid) {
        String query = "MATCH (n: user {uid: '%s' }) RETURN n.longitude, n.latitude, n.street, n.is_driver, n.uid";
        query = String.format(query, uid);
        return this.session.run(query);
    }
    ...
    public Result getDrivers() {
        String query = "MATCH (n: user {is_driver: true }) RETURN n.longitude, n.latitude, n.street, n.uid";
        return this.session.run(query);
    }
    ...
    public Result getRoute(String roadname1, String roadname2) {
        String query = "MATCH ({name : '%s'})-[r]->({name : '%s'}) RETURN r.travel_time, r.is_traffic";
        query = String.format(query, roadname1, roadname2);
        return this.session.run(query);
    }

    public Result deleteRoute(String roadname1, String roadname2) {
        String query = "MATCH (r1:road {name: '%s'})-[r:ROUTE_TO]->(r2:road {name: '%s'}) DELETE r";
        query = String.format(query, roadname1, roadname2);
        return this.session.run(query);
    }

    public Result getshortestpath(String roadname1, String roadname2){
        String query = "MATCH (start:road {name:'%s'}), (end:road{name:'%s'}) MATCH p=shortestPath((start)-[:ROUTE_TO*]->(end)) WITH nodes(p) AS nodes UNWIND nodes AS result RETURN result.name, result.is_traffic";
        query = String.format(query, roadname1, roadname2);
        return this.session.run(query);
    }
