...
public class location extends Endpoint {
    ...
    @Override
    public HttpResponse<String> sendRequest(String endpoint, String method, String reqBody)
    ...
    /* @Override
    public HttpResponse<String> sendRequest(String endpoint, String method, JSONObject reqBody)
            throws IOException, InterruptedException {
        HttpClient client = HttpClient.newHttpClient();
        HttpRequest request = HttpRequest.newBuilder()
                .uri(URI.create("http://localhost:8000" + endpoint))
                .method(method, HttpRequest.BodyPublishers.ofString(reqBody.toString()))
                .build();
